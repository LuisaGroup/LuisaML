...
class FeaturePreprocessor(nn.Module):
    """特征预处理模块，将输入特征转换为Transformer序列格式"""
    
    def __init__(self):
        super(FeaturePreprocessor, self).__init__()
        
    def forward(self, X_Val, X_Val_Sub, X_Val_Hg, scatterrate, g, gamma):
        """
        输入特征预处理：
        - X_Val: [batch, 192] 主特征
        - X_Val_Sub: [batch, 192] 子特征  
        - X_Val_Hg: [batch, 192] HG特征
        - scatterrate: [batch, 3] 散射率
        - g: [batch] 散射参数
        - gamma: [batch] 角度参数
        
        输出：
        - sequence: [batch, 192, 6] Transformer输入序列（192x6）
        """
        batch_size = X_Val.shape[0]
        
        # 只取scatterrate的第一个系数repeat 192次
        scatterrate_first = scatterrate[:, 0:1]  # [batch, 1] 取第一个系数
        scatterrate_expanded = scatterrate_first.repeat(batch_size, 1)  # [batch,1]
        
        # g和gamma从标量扩展到192维
        gamma = torch.tensor(gamma, device=X_Val.device, dtype=X_Val.dtype)
        g = torch.tensor(g, device=X_Val.device, dtype=X_Val.dtype)
        g_expanded = g.repeat(batch_size, 1)  # [batch,1]
        gamma_expanded = gamma.repeat(batch_size, 1)  # [batch,1]
        
        # 将6个特征拼成192x6的序列
        global_fea = torch.cat([scatterrate_expanded, g_expanded, gamma_expanded], dim=-1)  # [batch, 3]
        #import pdb;pdb.set_trace()
        sequence = torch.cat([
            X_Val.unsqueeze(1),                    # [batch, 1, 192]
            X_Val_Sub.unsqueeze(1),                # [batch, 1, 192]
            X_Val_Hg.unsqueeze(1),                 # [batch, 1, 192]
        ], dim=1)  # [batch, 3, 192]
        
        return sequence, global_fea


class TransformerEncoderBlock(nn.Module):
    """Transformer编码器块"""
    
    def __init__(self, d_model=64, nhead=8, dim_feedforward=256, dropout=0.1):
...
class FeatureProjection(nn.Module):
    """特征投影层，将输入特征映射到Transformer的d_model维度"""
    
    def __init__(self, input_dim=6, d_model=64):
        super(FeatureProjection, self).__init__()
        self.projection_val = nn.Linear(input_dim, d_model)
        self.projection_val_sub = nn.Linear(input_dim, d_model)
        self.projection_val_hg = nn.Linear(input_dim, d_model)
        self.layer_norm = nn.LayerNorm(d_model)
        self.projection_1 = nn.Linear(3, d_model)
        self.projection_2 = nn.Linear(3, d_model)
        self.projection_3 = nn.Linear(3, d_model)
        
    def forward(self, x, global_fea):
        """
        输入: x [batch, seq_len, input_dim]
        输出: [batch, seq_len, d_model]
        """
        #import pdb;pdb.set_trace()
        x_val = self.layer_norm(self.projection_val(x[:,0,:]))
        x_val_sub = self.layer_norm(self.projection_val_sub(x[:,1,:]))
        x_val_hg = self.layer_norm(self.projection_val_hg(x[:,2,:]))
        global_fea_1 = self.layer_norm(self.projection_1(global_fea))
        global_fea_2 = self.layer_norm(self.projection_2(global_fea))
        global_fea_3 = self.layer_norm(self.projection_3(global_fea))
        x = torch.cat([(x_val+global_fea_1).unsqueeze(1), (x_val_sub+global_fea_2).unsqueeze(1), (x_val_hg+global_fea_3).unsqueeze(1)], dim=1)
        return x


class SequencePooling(nn.Module):
    """序列池化层，对192长度的序列进行池化得到最终特征"""
    
    def __init__(self, d_model=64, pooling_type='mean'):
    ...
    def forward(self, x):
        ...
        if self.pooling_type == 'mean':
            ...
            return x.max(dim=1)[0]  # 最大池化
        elif self.pooling_type == 'attention':
            # 注意力池化
            attention_weights = torch.softmax(
                torch.matmul(x, x.transpose(1, 2)).mean(dim=-1), dim=-1
            ).unsqueeze(-1)
...
class TransformerMRPNN(nn.Module):
    """基于Transformer的MRPNN网络"""
    ...
    def __init__(self, 
                 ...
                 dropout=0.1,
                 pooling_type='mean'):
        super(TransformerMRPNN, self).__init__()
        
        # 特征预处理
        self.feature_preprocessor = FeaturePreprocessor()
        
        # 特征投影
        self.feature_projection = FeatureProjection(input_dim, d_model)
        ...
        # 序列池化
        self.sequence_pooling = SequencePooling(d_model, pooling_type)
    ...
    def forward(self, X_Val, X_Val_Sub, X_Val_Hg, scatterrate, g, gamma):
        ...
        """
        # 特征预处理
        sequence, global_fea = self.feature_preprocessor(X_Val, X_Val_Sub, X_Val_Hg, 
                                           scatterrate, g, gamma)
        
        # 特征投影到d_model维度
        x = self.feature_projection(sequence, global_fea)
        ...
        # 序列池化得到全局特征
        global_features = self.sequence_pooling(x)
...
def create_transformer_mrpnn_model(
    input_dim: int = 192,
    d_model: int = 32,
    nhead: int = 4,
    num_layers: int = 4,
    ...
    """
    创建Transformer MRPNN模型
    
    Args:
        input_dim: 输入特征维度（6个特征拼接）
    ...
    Returns:
        TransformerMRPNN模型实例
    """
    model = TransformerMRPNN(
        input_dim=input_dim,
        d_model=d_model,
        nhead=nhead,
...
def train_transformer_mrpnn(
    ...
    num_epochs: int = 50,
    lr: float = 1e-3,
    scatterrate_default: Optional[torch.Tensor] = None,
    g_default: Optional[torch.Tensor] = None,
    gamma_default: Optional[torch.Tensor] = None
):
    """
    训练Transformer MRPNN模型
    
    Args:
        model: Transformer MRPNN模型
        train_loader: 训练数据加载器
        val_loader: 验证数据加载器
        device: 训练设备
        num_epochs: 训练轮数
        lr: 学习率
        scatterrate_default: 默认散射率参数
        g_default: 默认散射参数
        gamma_default: 默认角度参数
    ...
    # 设置默认参数
    if scatterrate_default is None:
        scatterrate_default = torch.tensor([0.8, 0.8, 0.8], device=device).float()
    if g_default is None:
        g_default = torch.tensor(0.1, device=device).float()
    if gamma_default is None:
        gamma_default = torch.tensor(0.04, device=device).float()
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch_data in enumerate(train_loader):
            # 准备输入数据
            X_Val = batch_data['x_val'].to(device)
            X_Val_Sub = batch_data['x_val_sub'].to(device)
            X_Val_Hg = batch_data['x_val_hg'].to(device)
            targets = batch_data['ref'].to(device)
            
            # 获取批次大小并扩展散射参数
            batch_size = X_Val.shape[0]
            scatterrate = scatterrate_default.unsqueeze(0).repeat(batch_size, 1)
            g = g_default.unsqueeze(0).repeat(batch_size)
            gamma = gamma_default.unsqueeze(0).repeat(batch_size)
            
            # 前向传播
            optimizer.zero_grad()
            outputs = model(X_Val, X_Val_Sub, X_Val_Hg, scatterrate, g, gamma)
...
class FinalFusion(nn.Module):
    ...
    def __init__(self):
        ...
        '''
        self.fusion_layers_1 = LinearReLU(128, 128)
        self.fusion_layers_2 = LinearReLU(128, 64)
        self.fusion_layers_3 = LinearReLU(64, 32)
        self.fusion_layers_4 = LinearReLU(32, 16)
        self.fusion_layers_5 = LinearReLU(16, 16)
        self.fusion_layers_6 = LinearReLU(16, 16)
        self.fusion_layers_7 = LinearReLU(16, 16)
        self.fusion_layers_8 = LinearReLU(16, 16)
        self.fusion_layers_9 = LinearReLU(16, 16)
        self.fusion_layers_10 = LinearReLU(16, 16)
        self.fusion_layers_11 = LinearReLU(16, 16)
        self.fusion_layers_12 = LinearReLU(16, 16)
        self.fusion_layers_13 = LinearReLU(16, 16)
        self.fusion_layers_14 = LinearReLU(16, 16)
        self.fusion_layers_15 = LinearReLU(16, 16)
        self.fusion_layers_16 = LinearReLU(16, 16)
    ...
    def forward(self, comb_features, di_features, X_ValA, X_ValB, X_ValC, avg_pool, sc_base, g, gamma):
        ...
        X_ValB = torch.cat([X_ValB_new, X_ValB[:, 128:]], dim=1)
        X_ValA_new = self.fusion_layers_2(X_ValB[:, :128])
        X_ValA = torch.cat([X_ValA_new, X_ValA[:, 64:]], dim=1)
        X_ValB_new = self.fusion_layers_3(X_ValA[:, :64])
        X_ValB = torch.cat([X_ValB_new, X_ValB[:, 32:]], dim=1)
        X_ValA_new = self.fusion_layers_4(X_ValB[:, :32])
        ...
        X_ValA = torch.cat([X_ValA_new, X_ValA[:, 16:]], dim=1)
        X_ValB_new = self.fusion_layers_7(X_ValA[:, :16])
        X_ValB = torch.cat([X_ValB_new, X_ValB[:, 16:]], dim=1)
        X_ValC_new = self.fusion_layers_8(X_ValB[:, :16])
        X_ValC = torch.cat([X_ValC_new, X_ValC[:, 16:]], dim=1)

        X_ValA_new = X_ValA[:, :16] + X_ValC[:, :16]
        X_ValA = torch.cat([X_ValA_new, X_ValA[:, 16:]], dim=1)
        X_ValB_new = self.fusion_layers_9(X_ValA[:, :16])
        X_ValB = torch.cat([X_ValB_new, X_ValB[:, 16:]], dim=1)
        X_ValC_new = self.fusion_layers_10(X_ValB[:, :16])
        X_ValC = torch.cat([X_ValC_new, X_ValC[:, 16:]], dim=1)


        X_ValA_new = X_ValA[:, :16] + X_ValC[:, :16]
        X_ValA = torch.cat([X_ValA_new, X_ValA[:, 16:]], dim=1)
        X_ValB_new = self.fusion_layers_11(X_ValA[:, :16])
        X_ValB = torch.cat([X_ValB_new, X_ValB[:, 16:]], dim=1)
        X_ValC_new = self.fusion_layers_12(X_ValB[:, :16])
        X_ValC = torch.cat([X_ValC_new, X_ValC[:, 16:]], dim=1)

        X_ValA_new = X_ValA[:, :16] + X_ValC[:, :16]
        X_ValA = torch.cat([X_ValA_new, X_ValA[:, 16:]], dim=1)
        X_ValB_new = self.fusion_layers_13(X_ValA[:, :16])
        X_ValB = torch.cat([X_ValB_new, X_ValB[:, 16:]], dim=1)
        X_ValC_new = self.fusion_layers_14(X_ValB[:, :16])
        X_ValC = torch.cat([X_ValC_new, X_ValC[:, 16:]], dim=1)

        X_ValA_new = X_ValA[:, :16] + X_ValC[:, :16]
        X_ValA = torch.cat([X_ValA_new, X_ValA[:, 16:]], dim=1)
        X_ValB_new = self.fusion_layers_15(X_ValA[:, :16])
        X_ValB = torch.cat([X_ValB_new, X_ValB[:, 16:]], dim=1)
        X_ValC_new = self.fusion_layers_16(X_ValB[:, :16])
        X_ValC = torch.cat([X_ValC_new, X_ValC[:, 16:]], dim=1)
